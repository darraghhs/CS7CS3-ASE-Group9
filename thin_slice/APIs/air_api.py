...
def display_air_quality(location, air_data):
    ...
    aqi_level, aqi_icon = get_aqi_level(aqi_value)

    print("📊 EUROPEAN AIR QUALITY INDEX (AQI)")
    print(f"   {aqi_icon} AQI Level: {aqi_level} ({aqi_value})\n")
    ...
    if pm25 <= 12:
        ...
        print("   ⚠️  PM2.5 above WHO guidelines (> 12 µg/m³)")
    ...
    if aqi_level == "Good":
        ...
        print(
            "   ⚠️  Air quality is acceptable - sensitive groups may "
            "experience minor effects"
        )
    elif aqi_level == "Moderate":
        print("   ⚠️  Air quality is unhealthy for sensitive groups")
        print(
